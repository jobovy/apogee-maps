...
dat = ldata
print 'Fitting high alpha mono-[Fe/H] mono-age populations...'
for j in tqdm(range(0,len(fehbins)-1)):
    paramtfeh = []
    samplesfeh = []
    numbinfeh = []
...
savfile = open('../out/paramsRGB_brokenexpflare_01dex2gyrbins_high.dat', 'w')
...
dat = ldata
print 'Fitting low alpha mono-[Fe/H] mono-age populations...'
for j in tqdm(range(0,len(fehbins)-1)):
    paramtfeh = []
    samplesfeh = []
    numbinfeh = []
...
samples = numpy.array(samples)
obj = [agebins, fehbins, numbins, paramt, samples]
savfile = open('../out/paramsRGB_brokenexpflare_01dex2gyrbins_low.dat', 'w')
pickle.dump(obj, savfile)
savfile.close()
